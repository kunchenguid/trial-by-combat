export const RULES = {
  ...
  resolutionOrder: [
    {
      label: 'Heal / Scan / Dash setup',
      detail:
        'Heals and scans apply. Dash spends 1 charge. (You cannot heal AFTER seeing an attack land - heals resolve before damage.)',
    },
    {
      label: 'Movement',
      detail:
        "Both sides' moves resolve simultaneously. Same target → both blocked. Swap → both blocked. Pick your move assuming the opponent also moves this turn.",
    },
    {
      label: 'Relic auto-pickup (post-move)',
      detail:
        'A free relic is picked up by a sole occupant of its tile right after movement, BEFORE attacks resolve. So an attacker adjacent to the relic can damage the new carrier the same turn and force a drop.',
    },
    {
      label: 'Attack',
      detail:
    ...
    {
      label: 'Place wall / trap',
      detail: "Adjacent empty tile (orthogonal or diagonal). Walls can't seal off relic or bases. Traps are permanent.",
    },
    { label: 'Buff pickup', detail: 'Buff tiles are picked up by a single occupant; consumed on pickup.' },
    {
      label: 'End-of-turn relic pickup',
      detail: 'Final pickup pass for relics dropped during this turn (e.g. on knockout).',
    },
  ],
      ...
      items: [
        {
          ...
          sprite: null,
          effect:
            '2 dmg to opponent ORTHOGONALLY adjacent (N/S/E/W only - diagonals do NOT count). 5 dmg if you started in a bush. +1 vs relic carrier.',
        },
      ...
      items: [
        {
          ...
          sprite: 'tool_wall',
          effect:
            'Adjacent tile (orthogonal or diagonal) becomes impassable. Bushes are valid targets (the wall replaces the bush). Cannot target tiles that have a wall, trap, buff, the relic, a base, or a player.',
        },
        {
          name: 'Place Trap',
          sprite: 'tool_trap',
          effect:
            "Hidden trap on adjacent tile (orthogonal or diagonal). 5 dmg, cancels rest of enemy turn, AND stuns them for their next turn (forced WAIT, no respawn). PERMANENT - stays armed and re-triggers on every enemy step. Bushes are valid targets (a trap hidden in a bush is especially sneaky). Cannot target tiles that have a wall, trap, buff, the relic, a base, or a player. Tip: if you can predict the opponent's next move, place a trap on their path to cause damage.",
        },
      ],
    },
  ],
  ...
  tiles: [
    ...
    { name: 'Fire', sprite: 'icon_fire', effect: '-2 HP when you walk onto it.' },
    {
      ...
      sprite: 'icon_trap',
      effect:
        '-5 HP on enemy step, cancels rest of their turn, and stuns them for their next turn (forced WAIT). Hidden. PERMANENT - re-triggers on every enemy step.',
    },
